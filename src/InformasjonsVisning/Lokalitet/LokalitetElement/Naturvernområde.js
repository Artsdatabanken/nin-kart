...
const NaturvernOmråde = ({
  ...
  return (
    <>
      ...
      <NinCard
        image="Naturvernområde"
        title={språk(tittel)}
        heading={språk(overordnet[0].tittel)}
        canExpand={false}
        hasData
      >
        {expanded => (
              ...
              <CardActionArea onClick={() => onNavigate(url)}>
                ...
                <CardContent>
                  <Typography variant="body2">
                    {props.revisjon?.dato?.førstvernet && (
                      <>
                        Først vernet:{" "}
                        {props.revisjon.dato.førstvernet.substring(0, 10)}
                        <br />
                      </>
                    )}
                  </Typography>
